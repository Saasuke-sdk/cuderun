...
const tsFilePath = path.join('src', `Carrun.ts`);
const cairoFilePath = path.join(__dirname, '..', 'contract', 'src', `Carrun.cairo`);
...
try {
  ...
  libContent = libContent.replace('{{Caironame}}', 'Carrun');
  deployContent = deployContent.replace('{{Caironame}}', 'Carrun');
  ...
  console.log(`lib.cairo updated with module name: Carrun`);
